...
export async function getStockDetails(symbol: string) {
  try {
    ...
    const quote = await fetchJSON<QuoteData>(quoteUrl, 60); // Cache for 1 minute

    // Fetch profile (company name, market cap)
    const profileUrl = `${FINNHUB_BASE_URL}/stock/profile2?symbol=${encodeURIComponent(upperSymbol)}&token=${token}`;
    const profile = await fetchJSON<ProfileData>(profileUrl, 3600); // Cache for 1 hour

    // Fetch financials (basic metrics)
    const financialsUrl = `${FINNHUB_BASE_URL}/stock/metric?symbol=${encodeURIComponent(upperSymbol)}&metric=all&token=${token}`;
    const financials = await fetchJSON<FinancialsData>(financialsUrl, 3600); // Cache for 1 hour
  ...
  async (query?: string): Promise<StockWithWatchlistStatus[]> => {
    try {
      ...
      try {
        ...
        // User not authenticated, watchlist stays empty
        console.log("No session for watchlist lookup");
      }
